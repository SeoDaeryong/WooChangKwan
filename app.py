...
def woochangkwan_api(event, target, start=0):
    ...
    for item in items[start:start+10]:
        ...
        bubble_string += str(template).replace("'", "\"").replace("True", "true")
...
def handle_text_message(event):
    ...
    if text == 'profile':
        if isinstance(event.source, SourceUser):
            ...
            line_bot_api.reply_message(
                event.reply_token, [
                    ...
                    TextSendMessage(text='User ID: ' + str(event.source.user_id))
                ]
            )
        else:
            line_bot_api.reply_message(
        ...
        line_bot_api.reply_message(event.reply_token, template_message)
    elif text == 'carousel_new':
        carousel_template_new = CarouselTemplate(columns=[
            CarouselColumn(thumbnail_image_url='https://imgnews.pstatic.net/image/thumb/5353/2020/02/02/625914.jpg',
                           text='신종 코로나바이러스(우한폐렴) 여파로 BTS 회사 설명회 취소 "건강·안전 최우선"',
                           actions=[
                               URIAction(label='원문 보기', uri='http://www.topstarnews.net/news/articleView.html?idxno=726637'),
                               PostbackAction(label='관련 기사 더보기', data='ping')
                           ]),
            CarouselColumn(thumbnail_image_url='https://imgnews.pstatic.net/image/thumb/421/2020/02/02/4438715.jpg',
                           text='\'아는 형님\' 슬리피 "BTS 진, 힘들면 도와준다고 연락"',
                           actions=[
                               URIAction(label='원문 보기',
                                         uri='https://news.naver.com/main/read.nhn?mode=LSD&mid=sec&sid1=106&oid=421&aid=0004438715'),
                               PostbackAction(label='관련 기사 더보기', data='ping')
                           ]),
        ])
        template_message = TemplateSendMessage(
            alt_text='Carousel new alt text', template=carousel_template_new)
        line_bot_api.reply_message(event.reply_token, template_message)
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('-p', '--port', type=int, default=os.getenv("PORT", 5000), help='port')
